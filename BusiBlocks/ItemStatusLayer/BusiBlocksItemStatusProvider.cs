...
{
    public class BusiBlocksItemStatusProvider : ItemStatusProvider
    {
        ...
        public override ItemStatus GetItemStatusByName(string name)
        {
            using (var transaction = new TransactionScope(_configuration))
            {
                var store = new ItemStatusDataStore(transaction);
                return store.FindByName(name);
            }
        }

        public override ItemStatus GetItemStatus(string id)
        {
            using (var transaction = new TransactionScope(_configuration))
            {
                var store = new ItemStatusDataStore(transaction);
