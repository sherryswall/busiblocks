...
namespace BusiBlocks.DocoBlock
{
    ...
    public class ArticleDataStore : EntityDataStoreBase<Article, string>
    {
        ...
        public Article FindByName(string name)
        {
            ...
            criteria.Add(Expression.Or(Expression.Eq("Deleted", false), Expression.IsNotNull("Deleted")));

            return FindUnique(criteria, false);
        }

        public IList<Article> FindAllArticles()
        {
            ICriteria criteria = CreateCriteria();
        ...
        public IList<Article> FindByOwner(string owner, ArticleStatus status)
        {
            ICriteria criteria = CreateCriteria();
            criteria.AddOrder(Order.Asc("Title"));
        ...
        public IList<Article> FindByGroup(string groupId)
        {
            ICriteria criteria = CreateCriteria();
            criteria.Add(Expression.InsensitiveLike("Groups", "%" + groupId + "%"));
            AddCriteriaForStatus(criteria, ArticleStatus.EnabledAndApproved);
            criteria.AddOrder(Order.Desc("UpdateDate"));
