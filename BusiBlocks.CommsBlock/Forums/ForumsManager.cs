...
namespace BusiBlocks.CommsBlock.Forums
{
    ...
    {
        static ForumsManager()
        {
            //Get the feature's configuration info
            ForumsProviderConfiguration qc =
                (ForumsProviderConfiguration)ConfigurationManager.GetSection("forumsManager");

            if (qc == null || qc.DefaultProvider == null || qc.Providers == null || qc.Providers.Count < 1)
                throw new ProviderException("You must specify a valid default provider for forumsManager.");

            //Instantiate the providers
            providerCollection = new ForumsProviderCollection();
            ProvidersHelper.InstantiateProviders(qc.Providers, providerCollection, typeof(ForumsProvider));
            providerCollection.SetReadOnly();
            defaultProvider = providerCollection[qc.DefaultProvider];
            if (defaultProvider == null)
            {
                throw new ConfigurationErrorsException(
                    "You must specify a default provider for the ForumsManager.",
        ...
        //Public feature API
        private static ForumsProvider defaultProvider;
        private static ForumsProviderCollection providerCollection;

        public static ForumsProvider Provider
        {
            get{return defaultProvider;}
        }

        public static ForumsProviderCollection Providers
        {
